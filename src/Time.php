...
class Time
{
    ...
    public static function lastHour()
    {
        list($y, $m, $d) = explode('-', date('Y-m-d'));
        $h = date('H');
        $begin = mktime($h, 0, 0, $m, $d, $y) - 3600;
        $end = mktime($h, 59, 59, $m, $d, $y) - 3600;
        return [$begin, $end];
    }

    /**
     * 返回今日开始和结束的时间戳
     * @param bool $zero 只返回零点
     * @return array|integer
     */
    public static function today($zero = false)
    {
        list($y, $m, $d) = explode('-', date('Y-m-d'));
        if($zero){
     ...
     * @return array
     */
    public static function monthByTime($time)
    {
     ...
     * @return integer
     */
    public static function secondOfWeek()
    {
        list($y, $m, $d, $w) = explode('-', date('Y-m-d-w'));
        if($w == 0) $w = 7; //修正周日的问题
        $endTime = mktime(23, 59, 59, $m, $d - $w + 7, $y);
        $second = $endTime - time();
        return $second < 0 ? 0 : $second;
    }
     ...
     * @return integer
     */
    public static function secondOfMonth()
    {
        list($y, $m, $t) = explode('-', date('Y-m-t'));
        $endTime = mktime(23, 59, 59, $m, $t, $y);
        $second = $endTime - time();
        return $second < 0 ? 0 : $second;
    }
